...
Color Buffer_scanline24[8 + 256 + 8];
				...
				)
{
	...
	for(i = 0; i < 64; i++)
	{
		uint8 r = Palette32[i].r;

		Palette32[i].r = Palette32[i].b;
		Palette32[i].b = r;
	}
	...
	//frameBuf/fer = (Color*)malloc(240 * 320 * sizeof(Color));
	...
	for(x_TitleLine = PPU_BG_HScrlOrg >> 3; x_TitleLine < 32; x_TitleLine++){		   //从左数第一个显示title单元开始
		...
		for(i=7; i>=0; i--){							   //先写左边像素的颜色
			...
			if(BG_color_num & 3){			  //如果低两位为0，则为透明色,不写入
				...
				//Buffer_scanline24[Buffer_LineCnt] = Palette32[PPU_Mem.image_palette[BG_color_num]];
				Buffer_LineCnt++;
			}else{
				Buffer_LineCnt++;
			}
...
void NES_RenderLine(int y_axes)
{
	...
	//mrc_bitmapShowEx(Buffer_scanline+8,0,y_axes,240,240,1,2,0,0);
	memcpy((char*)(LCDBUF+(y_axes*240)),(char*)(Buffer_scanline+8),240*2);
	//memcpy((char*)(frameBuffer+(y_axes*240)),(char*)(Buffer_scanline24+8),240*3);
}
